  ...
  void getData() async {
    try {
      ...
      querySnapshot.docs.forEach((doc) {
        ...
        if (data['พนักงานที่มอบหมายให้ดูแลแทน']['EmployeeID'] ==
            userData!['EmployeeID']) {
          mapData!.add(data);
        }
      });
      ...
      for (int i = 0; i < mapData!.length; i++) {
        print(i);
        DateTime first = mapData![i]!['วันที่เริ่ม'].toDate();
        DateTime last = mapData![i]!['วันที่สิ้นสุด'].toDate();
        DateTime now = DateTime.now();
        if (now.isAfter(first) && now.isBefore(last)) {
          print("วันนี้อยู่ระหว่างวันที่เริ่มและวันที่สิ้นสุด");
          checkDate.add(true);
          checkDateText.add('');
        } else {
          print("วันนี้ไม่อยู่ระหว่างวันที่เริ่มและวันที่สิ้นสุด");
          checkDate.add(false);

          if (now.isBefore(first)) {
            checkDateText.add('รอดำเนินการ');
          } else if (now.isAfter(last)) {
            checkDateText.add('หมดเวลา');
          }
        }
      }
                                      ...
                                      GestureDetector(
                                        onTap: !checkDate[i]
                                            ? () => Fluttertoast.showToast(
                                                ...
                                                msg:
                                                    'รายการนี้ไม่สามารถเปิดออเดอร์ได้ค่ะ')
                                            : () => Navigator.push(
                                                context,
                                                CupertinoPageRoute(
                                                  builder: (context) =>
                                                    ...
                                                    idEmployee: mapData![i]![
                                                        ...
                                                        ['EmployeeID'],
                                                    userIDOpen: mapData![i]![
                                                            'พนักงานที่ดูแลลูกค้า']
                                                        ['EmployeeID'],
                                                    userNameOpen:
                                                        '${mapData![i]!['พนักงานที่ดูแลลูกค้า']['Name']} ${mapData![i]!['พนักงานที่ดูแลลูกค้า']['Surname']}',
                                                    // userIDOpen:
                                                    //     userData!['EmployeeID'],
                                                    // userNameOpen:
                                                    //     '${userData!['Name']} ${userData!['Surname']}',
